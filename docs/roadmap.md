...
> 最后更新：2026-08-31 · 当前阶段：TODO 优化项 **T5–T11、T18 全部完成**；后续规划 **T19–T22 已立项**（性价比评估：定时回归通知 / A/B 对比 / xdist 并行 / 自动开草稿 PR）
...
1. **T19 定时回归 + 自愈失败通知**（cron 夜间回归 + webhook 摘要推送，让框架持续运行）；
2. **T20 自愈价值 A/B 对比演示**（T3b：同场景开关自愈双轮对比，核心卖点量化实证）；
3. **T21 pytest-xdist 并行兼容**（SQLite 并发写 / 会话聚合验证修复，显式声明依赖）；
4. **T22 修复建议自动开草稿 PR**（守 T15 人审边界，缩短闭环）。
