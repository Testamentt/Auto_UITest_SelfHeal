"""LLM / VLM 公共输入/输出基础设施（T10 自 agent/llm_io.py 归位至 LLM 抽象层）。

归属理由：这些工具是"模型通信的输入准备与输出编解码"（与具体 provider 无关，
但只服务于模型交互），随模型抽象层走最贴切——业务代码已全部改从本模块导入。
...
DOM 解析与稳定定位器生成复用 agent/dom 公共工具（不依赖 strategies，避免循环导入）。
"""
